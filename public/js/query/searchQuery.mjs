...
function searchNavigation(totalPagesProducts, currentPage) {
...
function NewSearch(page, search) {
    ...
    fetch(`/allProducts?page=${page}&q=${search}`, {
        method: "POST"
    }).then(res => res.json())
        .then(async (data) => {
            ...
            if (data.success) {
                ...
                if (paginationContainer) {
                    searchNavigation(totalPages, currentPage)
                }
                if (ProductsContainer) {
                    // ProductsContainer.innerHTML = ""
                        ...
                        ProductsContainer.innerHTML += ` <div class="col-lg-4 col-md-4 col-sm-6 mt-40">
                                                    ...
                                                    <div class="single-product-wrap">
                                                        <div class="product-image">
                                                            <a href="/product/${ProductsList[i].slug}/${ProductsList[i].id}">
                                                        ...
                                                        <div class="product_desc">
                                                            <div class="product_desc_info">
                                                                ...
                                                                <h4><a class="product_name limited-text" href="/product/${ProductsList[i].slug}/${ProductsList[i].id}">${ProductsList[i].title}</a></h4>
                                                                
                                                            </div>
                                                            <div class="add-actions">
                                                                <ul class="add-actions-link">
                                                                    <li class="add-cart active"><a href="https://wa.me/${whatsappNumber}?text=Hi,%20I'm%20contacting%20you%20by%20clicking%20on%20the%20whatsapp%20button%20on%20your%20website. %20https://www.hluxegift.com/product/${ProductsList[i].slug}/${ProductsList[i].id}%20I'm%20inquiring%20about%20the%20*${ProductsList[i].title}*%20My%20Name%20is%20..." target=_blank><i class="fa fa-whatsapp"></i> <b>Contact us</b> </a></li>
                    ...
                    WriteError(`<strong class="font__weight-semibold">Oops!</strong> No Item Match your search`)
                }
            }
...
const Search = GetParameters(window.location.href).get("q")
if(Search && Search !== ""){
const page = GetParameters(window.location.href).get("page")

if (page && page > 0) {
    NewSearch(page, Search)
} else {
    NewSearch(1, Search)
